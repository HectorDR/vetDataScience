...
import dist1 from "Media/photo_dist/dist1.png";
import uniform1 from "Media/photo_dist/uniform1.png";
import uniform2 from "Media/photo_dist/uniform2.png";
import bin1 from "Media/photo_dist/form_bin.png";
import bin2 from "Media/photo_dist/form_bin11.png";
import bin3 from "Media/photo_dist/bin_results.png";
import bin4 from "Media/photo_dist/bin_dist.png";
import binex1 from "Media/photo_dist/bin_excel.png";
import binex2 from "Media/photo_dist/bin_excel2.png";
import binex3 from "Media/photo_dist/bin_excel3.png";
import pois from "Media/photo_dist/form_poisson.png";
import pois2 from "Media/photo_dist/form_poisson2.png";
...
<div>
           <Helmet>
  <title>4- Cómo se distribuyen los datos y su utilidad</title>
  <meta name="description" content="Cómo se distribuyen los datos y su utilidad"/>
  <meta name='keywords'
  content='distribuciones, distribucion uniforme, bernoulli, binomial, poisson'/>
    </Helmet>
   <main>
 ...
 <section id = "tabla" className = "container">
        ...
        <SubChapter id = "discretas">
            ...
            (<a href="https://www.investopedia.com/terms/d/discrete-distribution.asp"><u>Young, 2021</u></a>) por ejemplo el total de animales nacidos
             vivos en una camada de cerdos (1,2,3,...,14) o la probabilidad de que una cerda esté vacía o preñada (0 o 1). Las probabilidades discretas tienen un
            ...
            <Paragraph texto = "Entonces, según la formula general de la probabilidad, la novilla blanca tiene ¼ o 25% de probabilidad de salir, esta es la misma probabilidad para cada una de las restantes novillas, donde:"/>
            <center><h4> 1/4+1/4+1/4+1/4 = 1 </h4></center>

            <Paragraph texto = "No podemos calcular la media, no hay varianza y no hay interés en hacer alguna predicción.   Como podemos ver en el gráfico anterior cada color tiene exactamente la misma probabilidad de ser escogido (25% c/u)."/>
...
<DownloadButton link = "https://github.com/HectorDR/vetDataScience/blob/hojas_excel/4.3.1_Dist_Uniforme.xlsx?raw=true"/> 
        ...
        <SubTitle title="4.3.2- Distribuciones discretas - Bernoulli"/>
        <Paragraph texto ="Es una distribución que consiste en un solo ensayo y únicamente dos posibles resultados -éxito o fracaso-.  En la distribución de Bernoulli no hay forma de predecir el resultado. El ejemplo típico es lanzar la moneda al aire. En un lanzamiento (experimento) solo hay una posibilidad entre dos cara o sello."/>
        <Paragraph texto =
            "Esta distribución tiene por características principales que solo acepta un ensayo y solo hay dos posibles resultados (fracaso o error) "/>

        <Paragraph texto ="La fórmula de la probabilidad de Bernoulli es:"/>
        <center><h4>Var(Y) = p *(1-p)</h4></center>
        <br />
        <Paragraph texto = " A menudo se utiliza la distribución de Bernoulli para determinar el resultado que se espera de un solo ensayo. Ejemplo: una vaca inseminada solo tiene dos posibilidades al chequear: preñada o vacía."/>
        ...
        <SubChapter id = "binomial">
            ...
            <Paragraph  texto = {<>
                <p>La distribución binomial es una secuencia de eventos Bernoulli (éxito-fracaso) de la misma naturaleza. Es decir la distribucion
                 binomial repite un experimento <b>n</b> veces y cuantifica los éxitos y los fracasos.</p>
                 <b>Características:</b>
                <ul>
            ...
            <p>La fórmula de la distribución binomial es:</p>
        ...
        <ImagePlus 
                ...
                texto = " "
                /> 
        <Paragraph texto = "Aproximadamente es del 33% la probabilidad de preñar 4 cerdas dentro de las 6 que se van a inseminar. Si calculamos la probabilidad de que las cerdas inseminadas queden preñadas, podemos ver la distribución de probabilidades en el siguiente gráfico y deducir que hay mayores probabilidades de preñar entre 3 y 5 animales con los actuales resultados."/>

        <ImagePlus 
                width = "40%"
                max_zoom = "60%"
                image = {bin4}
                texto = "Histograma de frecuencias distribución binomial "
                /> 
...
<SubChapter id = "binexcel">
    ...
    <Paragraph texto = {<><p>La función de Excel para calcular la distribución binomial se usa la función <b>DISTR.BINOM o BINOM.DIST</b> en inglés. Si revisamos la captura de pantalla que se encuntra en la parte inferior, primero nos ubicamos en la celda donde deseamos ver el valor (D10), y ahí llamamos la función (=DISTR.BINOM) y la función nos solicita el número de éxitos que se encuentran en la celda B10, luego los ensayos (C5), seguido la probabilidad (C6) y finalmente la opción verdadero (true) si queremos la función acumulada o falso(para el valor específico), en este 
    ...
    <ImagePlus 
                ...
                max_zoom = "60%"
                image = {binex3}
                texto = "Porcentajes de la probabilidad de éxito/fracaso "
                />  

    <Paragraph texto ="Como se observa en el recuadro superior, la columna de la derecha que fue la asignada para ver los resultados de la probabilidad, cada una de las combinaciones de éxito/fracaso esta dada por un porcentaje, por ejemplo la posibilidad de seleccionar 6 cerdas preñadas es tan solo del 7.5%. La suma de estas probabilidades debe dar 1 o = 100%"/>
...
<SubChapter id = "poisson">
    <SubTitle title = "4.3.4- Distribuciones discretas-Poisson "/>
    <Paragraph texto = "Cuando queremos saber la probabilidad de un determinado evento Y, que ocurre en un intervalo de tiempo, distancia, área o volumen definidos usamos la distribución de Poisson."/>
    <center><h2>Y∼℘(μ)</h2></center>
    <br/>
    ...
    <Paragraph texto = {<><b>
        <ul>
            <li>- Número de pacientes que necesitan cirugía en una clínica veterinaria por día (planeación de quirófano).</li>
            <li>-	Número células somáticas por mililitro en leche (identificación de problemas de mastitis clínica).</li>
            <li> -	Número de partos en una piara por mes (planificación de número de parideras en la finca).</li>

        </ul></b>
    </>}/>
<h4> <center>La fórmula de la distribución de Poisson es:</center></h4>
    <ImagePlus 
     width = "30%"
    max_zoom = "50%"
    image = {pois}
    ...
    <Paragraph texto = {<><p>Al igual que con la distribución Binomial, Excel nos permite calcular las probabilidades de esta distribución utilizando la
      función <b>DISTR.POISSON o POISSON.DIST</b> en inglés. Entonces si queremos construir este tipo de distribución en Excel, lo mas práctico es
       hacer una tabla con los valores para los cuales deseamos calcular la probabilidad e incluir la media. Como se muestra en la ilustración, nos
        ubicamos en la celda donde deseamos ubicar la probabilidad, se escribe la función <b>=POISSON.DIST</b>. Si observamos la captura de pantalla ubicada en la parte inferior La función nos va primeo a pedir que insertemos la probabilidad de que ocurra el evento. Esta está ubicada en la celda C7 (probabilidad de un parto a la semana), luego la media de partos semanales que está en la celda C4 y finalmente la opción acumulativa (TRUE o VERDADERO) o si no queremos acumulados FALSO (FALSE).</p></>}/>
    ...
    texto = "Poisson en Excel "
     ...
     <Paragraph texto = {<><p>
         Realizamos el mismo procedimiento para los demás valores (estiramos la función) tal como hicimos con la distribución
    ...
    max_zoom = "60%"
    image = {pois4}
    texto = "Probabilidades para x número de partos a la semana, siguiendo la distribución de Poisson "
     /> 
    <p><center><b>Utilizando el diagrama de barras podemos visualizar la curva de la distribucion de Poisson para el ejemplo anterior</b></center></p> <br />

     <ImagePlus 
     width = "50%"
    max_zoom = "70%"
    image = {pois5}
    texto = "Histográma de frecuencias para las probabilidades "
     /> 

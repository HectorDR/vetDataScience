...
<div id = "tabla" className = "container bg-light">
    <ContentsTable
      ...
      <Paragraph texto = {<><p>
        ...
        Cada uno de los factores a incluir en el análisis pueden contener varios niveles de la variable de interés (ejemplo: razas, regiones, año de 
        producción). En síntesis el análisis de varianza nos sirve para conocer cuales son los efectos principales y las interacciones de una o mas
        ...
        En esta última sección, veremos como con una sencilla herramienta como Excel sin conocimientos previos de programación, podemos utilizar el 
      ...
      <Paragraph texto = {<>
        ...
        producción de leche entre las fincas que están a su cargo. Para ello ha tomado los promedios de la producción ajustada de leche a 305 días de 
        39 animales en cada una de las 4 granjas. Se establecen entonces las hipótesis  de la siguiente manera:</p><br/>
        <ul>
      ...
      <Paragraph texto = {<>
        ...
        <p>Lo primero que se debe hacer tan pronto se abre la hoja de diálogo es seleccionar los datos de interés, para este caso los cuatro años
          durante los cuales se siguió el programa de mejoramiento genético, en este caso los datos están agrupados por columnas y tienen encabezados
     ...
     <ImagePlus
      ...
      <Paragraph texto = {<>
      <p>Podemos utilizar El ANAVA de dos factores cuando tenemos dos variables de tipo nominal y/o dicótomas (0,1) y una variable cuantitativa y cada
        valor de una de las variables nominales está en combinación con cada valor de la otra variable nominal (y/o dicótoma), es decir podemos 
        cuantificar la variable cuantitativa en función de las variables nominales (Macdonald 2009).<br/>
        Por ejemplo, en un una granja avícola existen dos sistemas de encasetamiento (jaula y piso) y se trabaja con tres líneas de ponedoras 
        (Hy-line, Babcock y Lohman), entonces podemos comparar la producción de huevos por la raza de las aves y por el sistema de alojamiento en que se
     ...
     <ImagePlus
        ...
        />
      <Paragraph texto = {<>
         <center><h4>Promedios de producción de leche por región y por lactancia</h4></center><br/>
         <p>La tabla superior muestra la manera de insertar los datos en Excel donde se presentan los promedios de la leche por región (filas) y por 
           lactancia (columnas). Es importante que cada celda de la tabla tenga su resultado. Para este tipo de análisis todos los datos deben estar 
           presente es decir cada región debe tener un promedio de leche para cada lactancia, de lo contrario Excel no podra generar el reporte. 
           Una vez la tabla este lista del menu <b>Data Analysis</b> seleccionamos la opción <b>ANOVA two factor without replication</b> o ANAVA para
          dos factores sin replicación, como se observa en el cuadro inferior.</p>
          </>}/>
      ...
      <ImagePlus
        ...
        />
      <Paragraph texto = {<>
         <center> <h4> Seleccionar la opción ANAVA dos factores sin replicación</h4> </center><br/><br/>
         <p>Una vez seleccionada la opción tenemos una hoja de diálogo que nos pregunta por la ubicación de la tabla, el nivel de significancia y la 
           ubicación donde queremos el reporte tal como se obseva en la tabla inferior.</p>
      </>}/>
      <ImagePlus
          image={twof3}
        ...
        />
        <Paragraph texto = {<>
          <center> <h4> Seleccionar el rango donde se ubica la tabla, el nivel de significancia y la ubicación del reporte </h4></center><br/>
          <p>Finalmente, una vez seleccionamos OK, Excel generará un reporte que tiene dos tablas. En la primera nos presenta los valores descriptivos
             (sumatoria, media y varianza) para los valores tabublados tanto por filas (regiones), como por columnas (lactancias). La segunda tabla 
             presenta el análisis de varianza.</p>
        </>}/>
      <ImagePlus
          image={twof4}
        ...
        <Paragraph texto = {<>
          ...
          <p>En el ANAVA del reporte generado por Excel se presentan primero los resultados para las filas (Rows) en este caso las regiones. El F de 
            las filas es 15.52  y el F crítico es de 2.01 y El valor de P es inferior a 0.0001 por tanto no podemos aceptar la  H<sub>0</sub> y 
            debemos aceptar que la producción de leche no es igual para las regiones. En cuanto a las columnas (Lactancias) de igual manera el F 
            calculado (40.92) es superior al F critico (2.58) y el valor de p es muy inferior al 0.05 del 𝛼 seleccionado, por lo que también podemos 
            decir que existen diferencias significativas entre las lactancias del 1 al 5 por tanto rechazamos la H<sub>0</sub>. En conclusión en 
            ambos casos existen diferencias significativas tanto para las lactancias como para las regiones por lo cual se recharazon las dos 
            H<sub>0</sub>.</p>
        </>}/>
     ...
     <center><SubTitle title = "ANAVA de dos factores"/></center>
      ...
      <Paragraph texto = {<>
        <p>Este tipo de ANAVA es frecuente utilizarla en diseño experimental (bloques aleatorios) y también cuando se trabaja con mediciones repetidas, 
          es decir, cuando una observación ha sido hecha en el mismo individuo mas de una vez (antes y después), por ejemplo, cuando estamos midiendo 
          parámetros fisiológicos o de producción  en un individuo antes y después de algún tratamiento o al aplicar un estresor como cambios de 
          temperatura o humedad en los galpones.<br/>Al igual que el ANAVA sin replicación, el ANAVA factorial con replicación se utiliza cuando 
          tenemos una variable medible (cuantitativa) y dos variables nominales (factores o efectos principales) que se hallan en todas las posibles 
          combinaciones. En este caso tenemos tres H<sub>0</sub> a considerar (Macdonald,2009):</p><br/>
        <ul>
        ...
        <p>Se recomienda que los tamaños de las muestras de los subgrupos sean iguales ya que esto facilita el análisis. Si queremos realizar este tipo 
          de prueba en  Excel  su herramienta “Data Analysis” lo permite siempre y cuando el diseño esté balanceado.<br />
         ...
         en el promedio logarítmico de células somáticas (SCC/ml) en cuatro subregiones lecheras. Para ello calculamos los promedios logarítmicos de 
         SCC en 6 hatos representativos de cada una de las cuatro regiones en donde se realizaron las actividades de educación y control. Entonces, 
         ...
         los resultados de los 6 hatos por cada región o sea 24 datos por año durante 4 años, para un total de 96 registros.</p>
      </>}/>
      <ImagePlus
          image={rep}
      ...
      <Paragraph texto = {<>
        ...
        <p>La tabla superior presenta el promedio logarítmico de SCC para cada uno de los hatos distribuidos por región para cada año en cuestión. 
          La tabla debe estar ordenada y adecuadamente balanceada (las mismas observaciones por variable) en este caso 6 por región y 24 por año. 
      ...
      <ImagePlus
        ...
        />
      <Paragraph texto = {<>
        <center> <h4> En el menú de diálogo seleccionar la opción "ANOVA: two-factor with replication" ANAVA dos factores con replicación </h4></center><br/>
        <p>Inmediatamente se abre una hoja de diálogo que nos pide seleccionar el rango de las celdas donde se encuentra la tabla (incluir los encabezados), 
          y posteriormente se debe escribir el número de filas por cada muestra, para este caso son 6 por región (las regiones están en las filas) se debe 
          tener en cuenta que son las mismas observaciones para cada muestra (región). Luego seleccionamos el 𝛼 (0.05) y finalmente el lugar donde se desea 
          generar el reporte, como siempre las opciones pueden ser dentro del página actual, en una nueva página o un archivo diferente.</p>
      </>}/>
       <ImagePlus
          image={rep3}
      ...
      <Paragraph texto = {<>
        <center> <h4> Selección de opciones para generar el ANAVA </h4> </center><br />
        <p>Una vez presionamos OK, Excel genera un reporte con toda la información (ver tabla inferior). El reporte esta dividido en dos tablas, 
          la primera  presenta el resumen descriptivo (numero de fincas, sumas, promedio y varianza) para cada región por año, así como para la totalidad, 
          y la segunda tabla presenta el ANAVA para las regiones (Sample), para los años (Columns) y para la interacción de las dos variables.</p>
      </>}/>
      <ImagePlus
          image={rep4}
          texto="Reporte generado por Excel"
        ...
        />
        <Paragraph texto = {<>
        <center> <h4> Reporte generado por Excel listo para su interpretación </h4> </center><br/>
        <p>Enfocandonos en los resultados del ANAVA, la primera fila (Sample) muestra los resultados de las filas  que para nuestro ejemplo serian las 
        regiones. Para este caso el valor de F (8.27) es mayor que el F crítico (2.71) y el P-value es inferior a 0.0001 (inferior al valor de 𝛼 = 0.05) 
        por tanto no podemos aceptar la H<sub>0</sub> y podemos confirmar con un 95% de confianza que las medias del SCC logarítmico son diferentes 
        entre regiones. En la siguiente fila tenemos el análisis de las columnas, en este caso el valor de F (0.89) es inferior al F crítico (2.71) y 
        el P-Value (0.44) es superior al 𝛼 por tanto no podemos rechazar la H<sub>0</sub> y podemos afirmar con 95% de confianza que las medias del 
        conteo SCC logarítmico no son diferentes a través de los años, por tanto las medidas sanitarias que se tomaron no se han reflejado en una mejora 
        en los valores de las células somáticas de los hatos de cada una de las regiones. Finalmente la última fila nos muestra el efecto de la 
      ...
      <Paragraph texto= {<><p>
        ...
        />
        <Paragraph texto  = {<><p>
          Donde el valor de Y esta dado por 𝛃<sub>0</sub> que es el intercepto o el valor de Y cuando X vale 0, 𝛃<sub>1</sub> es la constante 
          de la pendiente de X<sub>1</sub> que es la variable independiente o predictor y 𝛆 es el error. Este representa las diferencias entre el Y 
          real y el y estimado pues muy seguramente la relación no es una diagonal perfecta, sino que hay variaciones. Típicamente se asume que el error 
          es independiente de X.<br/>
          Usos la regresión lineal:  Como decíamos anteriormente este modelo es el mas sencillo de todos, sin embargo no por eso debe ser menospreciado 
          ya que ha mostrado ser de amplia utilidad. Entre ellos tenemos la relación entre algunos fármacos y las constantes fisiológicas como la 
          presión arterial o la frecuencia cardiaca y respiratoria.  También en el sector agrícola para medir impactos de fertilizantes y nutrientes 
          sobre un cultivo (aunque estos tambien pueden obdecer a relaciones cuadráticas como veremos posteriormente 
          (James et al.,2013; Macdonald, 2009).<br/>
          Tambien pueden ser de utilidad en en los rendimientos de un cultivo. Para ilustrar la regresión lineal simple, veamos el gráfico adaptado de 
          la tabla tomada  de V. Sellam and E. Poovammal (1996) que muestra la relación entre el regimen de lluvias anual y el volumen de cosecha de 
          arroz en la India.
        </p></>}/>
        ...
        />
        <Paragraph texto = {<>
          <center> <h4> En la gráfica podemos visualizar el regimen de lluvias anuales (X) por la producción de arroz en millones de toneladas (Y) </h4></center><br/>
          <p>Como se puede observar existe una tendencia decreciente clara donde a mayor cantidad de lluvias hay una disminución en la producción de arroz 
            anual.  Según Macdonald (2009) existen tres cosas que se pueden hacer con este tipo de datos.  La primera es una prueba de hipótesis para ver si 
            hay una asociación entre las dos variables. En este caso nos gustaría entender cuanto disminuye la cosecha por unidad de lluvia que cae. 
            La segunda es describir que tan íntimamente están relacionadas las dos variables. Esto usualmente se expresa con el R<sup>2</sup>. 
            Y la última cosa que se puede hacer es determinar la ecuación de la línea que pasa a través de la nube de puntos (como se ve en el gráfico) 
            calculando el intercepto y el coeficiente de inclinación (no lo hacemos nosotros lo hace Excel).</p><br/><br/>
            <h3>La Hipótesis Nula (H<sub>0</sub>)</h3><br/>
            <p>La hipótesis nula (H<sub>0</sub>) de la regresión lineal es que la diagonal de la línea de 
            mejor ajuste es igual a cero. Es decir, en la medida en que X aumenta de tamaño la variable asociada Y no aumenta o disminuye.<br/>
            La regresión lineal busca la línea que mejor ajusta a los puntos de los datos (ver línea roja del gráfico de la dispersión de la producción 
            de arroz vs régimen de lluvias). El método mas común es la regresión de cuadrados mínimos ordinarios. En este método el mejor ajuste está 
            definido como la línea que minimiza las distancias verticales  entre los puntos de los datos y la línea elevadas al cuadrado 
            (Ver gráfico inferior). Para un punto de X con un valor X1 y un valor de Y con un valor de Y1, la diferencia entre Y1 y el calculado 
            (el valor predicho de Y a X1) y luego elevado al cuadrado.<br/> Esta diferencia cuadrada es calculada para cada punto y la suma de estas 
            diferencias al cuadrado indican que tan bien ajustan los datos. La línea de regresión es aquella para la que la suma de estos cuadrados es 
            la mas pequeña (Macdonald 2009).</p>
        </>}/>
        <ImagePlus
          image={linr3}
